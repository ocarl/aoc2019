class IntCode:
    def __init__(self, input_list, in_val=None):
...
def test():
    ...
    ic = IntCode(input_list, in_val=8)
    ic.run()
    assert ic.output == 1
    ic = IntCode(input_list, in_val=9)
    ic.run()
    assert ic.output == 0

    input_list = [3,9,7,9,10,9,4,9,99,-1,8]
    ic = IntCode(input_list, in_val=7)
    ic.run()
    assert ic.output == 1
    ic = IntCode(input_list, in_val=9)
    ic.run()
    ...
    ic = IntCode(input_list, in_val=0)
    ic.run()
    assert ic.output == 0
    input_list = [3, 12, 6, 12, 15, 1, 13, 14, 13, 4, 13, 99, -1, 0, 1, 9]
    ic = IntCode(input_list, in_val=1)
    ic.run()
    assert ic.output == 1
...
if __name__ == '__main__':
    with open('input5.txt') as f:
        ic = IntCode([int(x) for x in f.read().split(',')], in_val=5)
        ic.run()
        print(ic.output)
